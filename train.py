...
def load_data (path, split=0.1):
    images = sorted(glob(os.path.join(path, "original_image", "*.png")))
    masks1 = sorted(glob(os.path.join(path, "mask", "leftMask", "*.png")))
    masks2 = sorted(glob(os.path.join(path, "mask", "rightMask", "*.png")))
...
if __name__ == "__main__":
    ...
    lr = 1e-5
    num_epochs = 10
    model_path = os.path.join("ct", "model.h5")
    csv_path = os.path.join("ct", "data.csv")

    """ Dataset """
    dataset_path = "C:\\Users\\bekri\\Downloads\\ct_images"
